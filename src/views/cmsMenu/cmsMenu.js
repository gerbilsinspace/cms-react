...
class Menu extends Component {
  ...
  handleSubmit(val) {
    ...
    firebase.push(`/menus/${match.params.id}/links/`, {'url': val.url, 'text': val.text});
    this.props.clearAddLinkToMenu();
  }

  render() {
    const { menu, menuId, submitFailed, urlErrors, textErrors, deleteLink, firebase } = this.props;

    const isError = this.state.error || (submitFailed && (urlErrors.required || textErrors.required));
    ...
    const linkList = isEmpty(menu.links) ? 'Please add a link' : Object.keys(menu.links).map((link, id) => {
      ...
      if (linkInfo.url.includes('://')) {
        return (
          <div key={id}>
            ...
            <button onClick={ () => {deleteLink(menuId, link, firebase)}}>Delete</button>
          </div>
        )
      }
